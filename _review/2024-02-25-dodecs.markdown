...
Using $\nu$, it's easy to generate all other middle points via scale, rotation, and translation. This is how it's done on the complex plane:
...
Remember that we use sums and products of points on the square-triangle grid to generate the vertices of a new deflation. This makes the two operations pivotal to the generation of the grid, so we need a way to perform them efficiently.
...
[^11]: $\varphi$ is actually the primitive twelfth root of unity itself: $\zeta_{12}$. However, I thought I'd keep things simple and assign a different name to each primitive root of unity that gets used in this post. You might have spotted the others: $\zeta$ is actually $\zeta_3$, $i$ is $\zeta_4$ and $\tau$ is $\zeta_6$.