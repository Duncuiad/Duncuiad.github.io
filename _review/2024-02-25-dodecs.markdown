...
- The original vertices of the basic square and triangle have to be in the grid generated by the basis. They are the points $0, 1, \tau, i, 1+i$ in the last picture of the previous section.
- The (signed) sum of points that are on the grid has to be on the grid.
...
- The grid is closed under (signed) addition and multiplication[^7].
- The grid contains the points $0, 1, \nu, \tau, i$[^8].

The reason for this shift is that it yields a structure that is quite *regular*[^9], just like the square and the triangular grids were. There will likely be more points in it than we needed to include the whole square-triangle grid, but this is not an issue for us because we only need it to provide a good coordinate system, and it will.

<h3>The Construction</h3>

In the picture that depicts the vertices of the deflation diagram, you can notice that everything is built using only four elementary points, beyond $0$. They are $1$, $\nu$, $i$, and $\tau$ (we also explicitly require them to be on the grid). Every other point is obtained by addition or multiplication of those four. This sounds like a good candidate for a basis, doesn't it? Sadly, it can't be one: the four vectors are actually dependent, since $\tau = 1 + i - \nu$.
...
<br>
<hr>
Since the complex product distributes over the sum, we can simply check that multiplying elements of the candidate basis together always yields points that lie on the grid generated by that same collection.
...
Checking that $\nu^2 = -i + \nu + 2i\nu$ isn't as immediate as the other equalities, but you can check it for example by equating the real and imaginary parts of the two sides of the equality.
...
Remember that we use sums and products of points on the square-triangle grid to generate the vertices of a new deflation. This makes sums and products pivotal to the generation of the grid, which means that we need an efficient way to perform the two operations.

Since the dodec grid is closed under addition and multiplication (of complex numbers), we could take two sets of coordinates, find what points they represent on some basis, add or multiply those points together and then find the coordinates that represent the result on the same basis. This procedure, transposed directly in code, would require introducing approximations in the first and second steps, preventing us from being able to perform the third and last one.
...
[^8]: Notice that we can drop the explicit requirement of $1+i$ being on the grid: its presence follows from $1$ and $i$ being included and the grid being closed under addition. Also notice how, compared to the previous set of properties, we had to explicitly require $\nu$ to be included.

[^9]: This kind of structure is called a *ring* in maths. In particular, this would be a *subring* of the complex numbers. This particular ring also happens to be a free *abelian group* (of finite rank), which is another name for a (free) $\mathbb{Z}$-module. It's this latter kind of structure that we're actually interested in when we talk about (regular) grids: the set of all points that can be reached by stepping (with *full steps*) along a finite collection of 'vectors'.
